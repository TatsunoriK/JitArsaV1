...
def filter_docs(found_docs, q, locked_province: str = None):
    ...
    # ✅ คำที่บ่งบอกอยากได้งานออนไลน์/ที่บ้าน
    ...
    # ✅ คำที่บ่งบอกไม่เอางานออนไลน์/ที่บ้าน — จับหลายรูปแบบ
    _not_online_kws = [
    ...
    # ✅ ตรวจ history ด้วย — ถ้าเคยบอก preference ไว้
    if not want_online and not want_not_online:
        pass  # ไม่มี preference ใหม่ ใช้ history แทนได้
    ...
    # ✅ ถ้ามี hard filter (ออนไลน์/ไม่ออนไลน์/ฟรี) ให้ search จาก docs ทั้งหมด
    # เพราะ vector อาจคืนแค่งานออนไลน์มาแล้ว filter หมด
    ...
    for d in search_pool:
        ...
        if want_not_online and is_online:
            continue
        # ✅ ถ้ามี skill filter ให้กรองด้วย
        if skill_kws:
